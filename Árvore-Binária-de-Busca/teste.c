...
FILE *arquivo_streams;
FILE *arquivo_programas;
...
void teste_insercao_streams(Stream** raiz, char** nomes_streams) {
    ...
    if(op==1){
        for (int i = 0; i < NUM_STREAMS; i++) {
            ...
            Cadastra_stream(raiz, nome, url, &flag);
    ...
    // Salvar no arquivo
    fprintf(arquivo_streams, "Inserção - Tempo: %.8fs, Média: %.8fs, Ordem: %d", 
...
void teste_busca_streams(Stream* raiz, char** nomes_streams) {
    ...
    // Salvar no arquivo
    fprintf(arquivo_streams, "Busca - Tempo: %.8fs, Média: %.8fs, Encontradas: %d/%d\n\n", 
...
void teste_insercao_programas(Stream* stream_raiz, char** nomes_programas, Apresentadores* Apresentador) {
    ...
    // Salvar no arquivo
    fprintf(arquivo_programas, "Inserção - Tempo: %.8fs, Média: %.8fs, Ordem: %d", 
...
void teste_busca_programas(Programas* raiz, char** nomes_programas) {
    ...
    // Salvar no arquivo
    fprintf(arquivo_programas, "Busca - Tempo: %.8fs, Média: %.8fs, Encontrados: %d/%d\n\n", 
...
void teste_completo() {
    for(int g=0; g<30;g++){
        ...
        arquivo_streams = fopen("resultados_streams.txt", "a");
        arquivo_programas = fopen("resultados_programas.txt", "a");
        
        if (!arquivo_streams || !arquivo_programas) {
            printf("Erro ao criar arquivos de resultados!\n");
            return;
        }
        
        fprintf(arquivo_streams, "=== RESULTADOS STREAMS ===\n\n");
        fprintf(arquivo_programas, "=== RESULTADOS PROGRAMAS ===\n\n");
        ...
        libera_apresentadores(apresentadores);
